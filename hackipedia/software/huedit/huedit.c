...
wchar_t ime_func_latin_alpha(int c) {
    ...
    switch (ime_func_latin_alpha_prev) {
        case 0:
            if (lc == 'a' || lc == 'e' || lc == 'i' || lc == 'o' || lc == 'u' || lc == 'c' || lc == 'n' || lc == 's' || lc == 'y' || lc == 'd' || lc == 't')
...
void ime_draw_latin_alpha(int y1,int y2) {
    ...
    attrset(A_NORMAL);
    mvaddstr(y1+0,ofsx,"Enter letter A/D/E/I/O/U/C/N/S/T/Y, then enter another for xform.");
    mvaddstr(y1+1,ofsx,"a=acute c=circumflex g=grave h=eth r=ring s=slash t=tilde u=umlaut");
    mvaddstr(y1+2,ofsx,"cc=cedilla sz=eszett th=thorn");
}
